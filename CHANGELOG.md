...
- **The `init` scaffold renders just its one page**: With cover, document TOC and the part divider all quiet by default, `markpublish init` followed by `markpublish build` now yields exactly the welcome chapter — the first PDF anyone builds shows their content and nothing else.
